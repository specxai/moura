...
import {
  mouraEvidenceName,
  mouraEvidenceTest,
} from "../src/test-support/moura-evidence.js";
...
describe("registry smoke", () => {
  it(
    mouraEvidenceName(
      "recognizes an exact registry dependency and rejects a local link",
      ["REQ-006/SCN-001/CASE-002"],
      "integration",
    ),
    async () => {
      ...
      try {
        ...
        await expect(
          verifyRegistryDependency(project, "0.1.2"),
        ).rejects.toThrow("dependency value 0.1.2");
      } finally {
